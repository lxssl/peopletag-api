...
// Lazy load hash functions to handle ES module imports in CommonJS context
let hashFunctions = null;
const getHashFunctions = async () => {
  if (!hashFunctions) {
    const hkdfModule = await import('@noble/hashes/hkdf.js');
    const sha256Module = await import('@noble/hashes/sha2.js');
    hashFunctions = {
 ...
 * @returns {Promise<Uint8Array>} Encryption key (32 bytes)
 */
const generateEncryptionKeyFromSignature = async (signature) => {
  if (!signature) {
    throw new Error('Signature is required');
  }

  try {
    // Get hash functions (dynamic import for ES module)
    const { hkdf, sha256 } = await getHashFunctions();
...
const verifyWalletSignature = async (req, res, next) => {
  try {
    ...
    try {
      encryptionKey = await generateEncryptionKeyFromSignature(signature);
    } catch (keyError) {
