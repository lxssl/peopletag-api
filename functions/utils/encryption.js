...
// Lazy load gcm to handle ES module import in CommonJS context
let gcmModule = null;
const getGcm = async () => {
  if (!gcmModule) {
    gcmModule = await import('@noble/ciphers/aes.js');
  }
  return gcmModule.gcm;
};
 ...
 */
const decryptProfile = async (encryptedData, key) => {
  ...
  try {
    // Get gcm function (dynamic import for ES module)
    const gcm = await getGcm();
