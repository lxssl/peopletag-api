...
if (!admin.apps.length) {
  try {
    ...
    if (serviceAccount) {
      ...
      admin.initializeApp({
        projectId: process.env.FIREBASE_PROJECT_ID || 'peopletag-demo-1'
      });
    }
  } catch (error) {
    console.error('Error initializing Firebase Admin:', error);
    // For development, you might want to use the client SDK
    throw new Error('Firebase Admin initialization failed. Please set FIREBASE_SERVICE_ACCOUNT environment variable.');
  }
}
